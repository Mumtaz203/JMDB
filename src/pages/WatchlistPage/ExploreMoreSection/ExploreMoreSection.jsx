// src/sections/ExploreMoreSection.jsx
import React, { useRef } from 'react';
import { SmallMovieCard } from '../../../components/SmallMovieCard';
import { useQuery, gql } from '@apollo/client';
import { movieImages } from "../../../movieAssets";

const GET_ALL_MOVIES_EXPLORE = gql`
  query GetAllMovies {
    getAllMovies {
      id
      title
...
const ExploreMoreSection = () => {
  ...
  const { loading, error, data } = useQuery(GET_ALL_MOVIES_EXPLORE);
  ...
  if (loading) return <p className="text-white text-center">Loading explore more movies...</p>;
  if (error) return <p className="text-red-500 text-center">Error loading explore more movies: {error.message}</p>;
